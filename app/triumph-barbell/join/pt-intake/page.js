...
export default function PTIntakePage() {
  ...
  const [submitted,  setSubmitted]  = useState(false) // track whether user tried to submit
  const [submitting, setSubmitting] = useState(false)
  const [error,      setError]      = useState(null)

  useEffect(() => {
    try {
      const raw = sessionStorage.getItem('triumph_join_payload')
      if (!raw) { router.replace('/triumph-barbell/join'); return }
      setPayload(JSON.parse(raw))
    } catch {
      router.replace('/triumph-barbell/join')
    }
  }, [router])
  ...
  async function handleSubmit(e) {
    ...
    if (!payload) return
    setSubmitted(true)
    if (emptyKeys.length > 0) return   // show inline errors, don't proceed
    setError(null)
    setSubmitting(true)
    try {
      // 1. Save intake responses to DB
      const intakeRes = await fetch('/api/triumph-barbell/join/pt-intake', {
        method:  'POST',
      ...
      // 2. Create Stripe checkout session with the full join form payload
      const checkoutRes = await fetch('/api/triumph-barbell/join/checkout', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      sessionStorage.removeItem('triumph_join_payload')
  ...
  return (
    <div className="min-h-screen bg-[#292929] flex flex-col items-center py-12 px-4">
      ...
      >
        ...
        <p className="text-center text-[11px] text-neutral-700">powered by <a href="https://ironkeyentry.com" target="_blank" rel="noopener noreferrer" className="underline"><strong>ironkey llc</strong></a> · © 2026</p>
      </form>
    </div>
  )
}
