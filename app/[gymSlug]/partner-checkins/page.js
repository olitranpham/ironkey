...
export default function PartnerCheckinsPage() {
  ...
  const [confirmRemove, setConfirmRemove] = useState(null) // profile object, or null
  ...
  async function confirmRemoveProfile() {
    ...
    try {
      const token = localStorage.getItem('ik_token')
      const res = await fetch(`/api/${gymSlug}/guest-passes/profiles/${confirmRemove.id}`, {
        method:  'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      })
        ...
        >
          ...
          {!profileLoading && selectedProfile && (
            <GuestProfilePanel
              ...
              source="partner"
              onDelete={() => { setRemoveError(null); setConfirmRemove(selectedProfile) }}
            />
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1c1c1c] border border-neutral-800 rounded-xl w-full max-w-sm p-6 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-semibold text-white">remove guest profile?</p>
                <p className="text-xs text-neutral-500">{confirmRemove.name}</p>
              </div>
            </div>
            <ul className="space-y-1.5 mb-5">
              {[
                'their partner check-in history at this gym will be permanently deleted',
