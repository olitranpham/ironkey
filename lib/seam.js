...
export async function deleteSeamCodeByPin(apiKey, pin, deviceId = null, logPrefix = '[seam]') {
  ...
  try {
    ...
    if (!delRes.ok) {
      ...
      console.error('%s deleteSeamCodeByPin: delete failed status=%s body=%s', logPrefix, delRes.status, text)
      return { ok: false, codeId: targetCodeId }
    }
...
export async function deleteSeamCodeById(apiKey, codeId, logPrefix = '[seam]') {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('%s deleteSeamCodeById: delete failed status=%s body=%s', logPrefix, res.status, text)
      return { ok: false }
    }
    console.log('%s deleteSeamCodeById: deleted access_code_id=%s', logPrefix, codeId)
    return { ok: true }
  } catch (err) {
