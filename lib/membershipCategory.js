// Groups PT/programming/group-training membershipType variants into general
// categories for display and filtering, without ever touching the stored
// value itself.
//
...
//     '1 session/week') go straight into membershipType. Group training uses
//     the identical tier-name format ('1 session/week' etc), so it's stored
//     with a 'group training: ' prefix to stay distinguishable from PT —
//     without that marker the two are literally the same string and
...
export const CATEGORY_OPTIONS = ['personal training', 'programming', 'group training']
...
export function classifyMembershipType(rawType) {
  ...
  if (!t) return null
  // Checked first — group training tiers otherwise read identically to PT's
  // own tier names ('1 session/week' etc) and would fall through to the
  // session/week rule below without this taking priority.
  if (t.includes('group training'))           return 'group training'
